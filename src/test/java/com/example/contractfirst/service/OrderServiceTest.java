 ...
 *
 * <p>Tests business logic with mocked dependencies.
 *
...
class OrderServiceTest {
  ...
  void createOrder_ShouldCreateNewOrder_WhenNoIdempotencyKey() {
    ...
    when(orderMapper.toEntity(anyString(), any(com.example.contractfirst.dto.OrderItem.class)))
  ...
  void getOrder_ShouldReturnOrder_WhenExists() {
    ...
    OrderResponse expectedResponse =
        new OrderResponse(orderId, "CUST-123", "CREATED", List.of(), java.time.Instant.now());
  ...
  void createOrder_ShouldPublishKafkaEvent_WithCorrectData() {
    ...
    CreateOrderRequest request =
        new CreateOrderRequest(
            "CUST-123", null, List.of(new com.example.contractfirst.dto.OrderItem("SKU-001", 2)));
    ...
    when(orderMapper.toEntity(anyString(), any(com.example.contractfirst.dto.OrderItem.class)))
        .thenReturn(itemEntity);
    when(orderItemRepository.saveAll(anyList())).thenReturn(List.of(itemEntity));
    when(orderMapper.toResponse(any(OrderEntity.class)))
        .thenReturn(
            new OrderResponse(
                "ORD-12345", "CUST-123", "CREATED", List.of(), java.time.Instant.now()));
