 ...
 *
 * <p>Wraps failed messages in DeadLetterEnvelope with debugging information: - Original topic,
 * partition, offset - Consumer group - Error details - Base64-encoded payload for replay
 *
...
public class DeadLetterQueueHandler {
  ...
  public void sendToDeadLetterQueue(
      ConsumerRecord<String, ?> record, String consumerGroup, Exception exception) {
    try {
      String payloadBase64 = encodePayload(record.value());

      DeadLetterEnvelope envelope =
          DeadLetterEnvelope.newBuilder()
              ...
              .setErrorMessage(
                  exception.getMessage() != null ? exception.getMessage() : "No error message")
      ...
      log.warn(
          "Sent message to DLQ: topic={}, dlqTopic={}, key={}, error={}",
          record.topic(),
          dlqTopic,
          key,
      ...
      log.error(
          "Failed to send message to DLQ: topic={}, key={}, originalError={}, dlqError={}",
          record.topic(),
          record.key(),
          exception.getMessage(),
          e.getMessage(),
